...
export function WatchProgressSection({ tvShowId, seasonNumber }: WatchProgressSectionProps) {
  ...
  const episodeSummary = formatWatchProgressEpisodeSummary(
    progress.watchedEpisodes,
    progress.totalEpisodes,
  );
  const percentDisplay = formatProgressPercentDisplay(progressPercent);
  const nextOrStatus = nextEpisodeLine
    ? t('details.watchProgress.nextEpisodePrefix', { line: nextEpisodeLine })
    : completionLabel;
  ...
  return (
    <View
      ...
      accessibilityLabel={t('details.watchProgress.accessibilitySummary', {
        summary: episodeSummary,
        percent: percentDisplay,
        nextOrStatus: nextOrStatus ?? '',
      })}
    >
      <AppText variant="subtitle" style={styles.title}>
        {t('details.watchProgress.title')}
      </AppText>
      ...
      {nextEpisodeLine ? (
        ...
        <View style={styles.nextBlock}>
          <AppText variant="caption" muted style={styles.nextLabel}>
            {t('details.watchProgress.statusLabel')}
          </AppText>
          <AppText variant="bodySmall" style={styles.nextValue}>
            {completionLabel}
          </AppText>
