...
const ptPath = join(root, 'stage2-pt.ts');
const valuePtPath = join(dirname(fileURLToPath(import.meta.url)), 'stage2-value-pt.json');
...
const valueTranslations = JSON.parse(readFileSync(valuePtPath, 'utf8'));
...
const stage2Pt = translateTree(stage2En);
writeFileSync(ptPath, `export const stage2Pt = ${serialize(stage2Pt)} as const;\n`);
