...
const esPath = join(root, 'stage2-es.ts');
const valueEsPath = join(dirname(fileURLToPath(import.meta.url)), 'stage2-value-es.json');
...
const valueTranslations = JSON.parse(readFileSync(valueEsPath, 'utf8'));
...
const stage2Es = translateTree(stage2En);
writeFileSync(esPath, `export const stage2Es = ${serialize(stage2Es)} as const;\n`);
