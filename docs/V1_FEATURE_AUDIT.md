...
| **Account recovery** | **Implemented:** Forgot-password + reset-password screens with backend contract |
| **Store / legal compliance** | **Blocker:** Privacy policy URL, support contact, store assets, production API URL not in repo |
| **Operational readiness** | **Partial:** Auth rate limiting implemented (backend); crash/error monitoring and production analytics baseline still missing |
...
| Field | Detail |
|-------|--------|
| **Current status** | ✅ **IMPLEMENTED** — Mobile forgot/reset screens + backend endpoints |
| **Classification** | 🟢 V1 REQUIRED |
...
| Field | Detail |
|-------|--------|
| **Current status** | ✅ **IMPLEMENTED** — Mobile handles `429` on auth flows |
| **Classification** | 🟢 V1 REQUIRED |
...
| Rate limiting | ✅ Implemented | 🟢 Required | Ship as-is (backend) |
