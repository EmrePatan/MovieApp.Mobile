...
describe('watch progress formatting', () => {
  it('formats watched episode counts for detail UI', () => {
    ...
    expect(formatWatchProgressEpisodeSummary(1, 1)).toBe('1 of 1 episodes');
  });

  it('formats progress percentage for detail UI', () => {
    expect(formatProgressPercentDisplay(50.4)).toBe('50% complete');
    expect(formatProgressPercentDisplay(0)).toBe('0% complete');
    expect(formatProgressPercentDisplay(100)).toBe('100% complete');
  });
